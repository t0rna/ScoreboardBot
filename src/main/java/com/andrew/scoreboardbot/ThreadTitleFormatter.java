...
public class ThreadTitleFormatter
{
    ...
    public String format(ScheduleGame game)
    {
        ...
        return switch(game.gameType().toUpperCase())
        {
            ...
            default -> game.isDoubleheader() ? away + " @ " + home + " GM " + game.gameNumber() + " - " + game.gameDate().withZoneSameInstant(ZoneId.of("America/New_York")).format(DATE_FORMAT) : away + " @ " + home + " - " + game.gameDate().withZoneSameInstant(ZoneId.of("America/New_York")).format(DATE_FORMAT);
        };
    }
}
